...
export type ColumnConfig<TData> = {
    ...
    badge?: {
        variant?:
            ...
            | 'destructive'
            | 'primary';
        color?: string; // e.g. 'emerald' | 'amber'
        truthy?: string; // label when value is truthy, e.g. '✓ Yes'
        falsy?: string; // label when value is falsy,  e.g. 'No'
        map?: Record<string, { label: string; color?: string }>; // e.g. roles
    };
    tags?: boolean; // comma-separated → multiple Badge chips
    muted?: boolean; // render as small muted text (e.g. dates)
    render?: (value: any, row: TData) => React.ReactNode; // escape hatch
};
...
export interface DataTableProps<TData extends { id: number | string }> {
    ...
    showRoute?: (id: number) => RouteDefinition<'get'>;
    editRoute?: (id: number) => RouteDefinition<'get'>;
    destroyRoute?: (id: number) => RouteDefinition<'delete'>;
    bulkDestroyRoute?: () => RouteDefinition<'post'>;
    exportRoutes?: {
        excel?: RouteDefinition<'get'>;
...
function buildColumns<TData>(
    ...
    return configs.map((cfg) => ({
        ...
        cell: ({ row }: { row: any }) => {
            ...
            if (cfg.render) {
                return cfg.render(value, row.original);
            }

            // Truthy/falsy badge (e.g. email_verified_at)
            ...
            // Tags: comma-separated string → multiple badges
            if (cfg.tags) {
                return (
            ...
            // Muted small text (e.g. dates, IDs)
            if (cfg.muted) {
                return (
...
export default function DataTable<TData extends { id: number | string }>({
    ...
    // ── Checkbox column (Dark mode fixed) ─────────────────────────────────────
    const checkboxCol: ColumnDef<TData> = {
        ...
        header: ({ table }) => (
            <Checkbox
                className="border-2 border-(--checkbox-border) data-[state=checked]:border-primary data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground data-[state=indeterminate]:bg-primary/50"
                checked={
    ...
    const actionsCol: ColumnDef<TData> | null =
        showRoute && editRoute && destroyRoute
            ? {
                  id: 'actions',
                  header: 'Actions',
                  enableSorting: false,
            ...
            <div className="flex flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
                    ...
                    <div className="flex shrink-0 items-center gap-1.5">
                        {hasExport && (
                            <div className="relative">
                                ...
                                {showExportDropdown && (
                                    <div className="absolute top-full right-0 z-50 mt-1 min-w-35 animate-in rounded-lg border border-border bg-background shadow-lg duration-200 fade-in-0 slide-in-from-top-2">
                                        {exportRoutes?.excel && (
                                            <Link
                                                href={exportRoutes.excel}
                                                className="flex items-center gap-2 rounded-t-lg border-b border-border/50 px-3 py-2 text-xs first:rounded-t-lg last:border-b-0 hover:bg-accent"
                                            >
                        ...
                        {createRoute && (
                            <Button
                                ...
                                onClick={() => router.visit(createRoute!())}
                            >
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border bg-background px-4 py-2 dark:bg-background">
                    <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        >
                            <SelectTrigger className="h-7 w-15.5 border border-input bg-background px-2 text-xs">
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
